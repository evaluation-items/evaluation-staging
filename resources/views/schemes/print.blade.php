...
@media print {
    ...
    .footer-wrapper {
       position: fixed;
        ...
        left: 0;
        right: 0;
        border-top: 1px solid #000; /* The line you wanted */
        padding-top: 5px;
        text-align: right;
        background: white;
        font-size: 12px;
    ...
    /* 3. Financial Table Layout Fix */
   table {
        width: 100%;
        border-collapse: collapse;
        table-layout: auto; /* Allows columns to adjust to text length */
        page-break-inside: auto;
    }


     tr {
       page-break-inside: avoid; /* Keeps a single row from splitting across pages */
    }
   ...
   td, th {
    ...
    th {
        ...
        text-align: left;
        width: 35%; /* Gives labels enough room so they don't squash */
    }
  /* FIX FOR FINANCIAL TABLE (Image 2 issue) */
    thead {
    ...
    .main-title {
        text-align: center;
        font-size: 20px; /* Increased Title size */
        font-weight: bold;
        margin-bottom: 20px;
...
<body>
   <div class="footer-wrapper">
    ...
    // 1. Remove any old footers if they exist
    const oldFooters = document.querySelectorAll('.injected-footer');
    oldFooters.forEach(f => f.remove());

    // 2. A4 page height in pixels (~1122px for 96dpi)
    ...
    for (let i = 1; i <= numberOfPages; i++) {
        ...
        // Calculate the position: bottom of each page
        // Subtract 20px to ensure it stays above the physical edge
        const topPosition = (i * pageHeight) - 30; 
        
        footer.style.top = topPosition + 'px';
        footer.innerHTML = 'Page ' + i;
