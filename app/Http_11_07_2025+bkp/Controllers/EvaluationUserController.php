...
class EvaluationUserController extends Controller
{
         ...
         if($validate->fails()) {
            return redirect()->back()->withError($validate->errors());
                 ...
                 $user = User::create([
                     ...
                     'dept_id' => $request->input('dept_id'), // default to 0 if not provided
                     'role' => $request->input('role'),
                     'password' => $request->input('password') ? Hash::make($request->input('password')) : null,
                     'role_manage' => $request->input('role_manage'),
                     'login_user' => 3
                 ]);
                 ...
                 User_user_role_deptid::create([
                    ...
                    $act['ip'] = $request->ip();
                    $act['activity'] = 'Evaluation User Create By Admin';
    ...
    public function update(Request $request,$id){
        ...
        if($validate->fails()) {
            ...
            $act['userid'] = Auth::user()->id;
            $act['ip'] = $request->ip();
            $act['activity'] = 'Evaluation User Update By Admin';
            $act['officecode'] = Auth::user()->dept_id;
            $act['pagereferred'] = $request->url();
            Activitylog::insert($act);
    ...
    public function destroy(Request $request,$id)
    {
        if (!is_null($id)) {
           ...
           $act['userid'] = Auth::user()->id;
           $act['ip'] = $request->ip();
           $act['activity'] = 'Evaluation User Delete By Admin';
           $act['officecode'] = Auth::user()->dept_id;
           $act['pagereferred'] = $request->url();
           Activitylog::insert($act);
