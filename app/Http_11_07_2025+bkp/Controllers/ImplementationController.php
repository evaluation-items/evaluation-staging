...
class ImplementationController extends Controller {
    ...
    public function updateInfo(Request $request){
        ...
        if(!$validator->passes()){
            return response()->json(['status'=>0,'error'=>$validator->errors()->toArray()]);
             ...
             $act['ip'] = $request->ip();
             $act['activity'] = 'Update Profile Detail by Concern Department';
    ...
    public function updatePicture(Request $request){
        ...
        if( !$upload ){
            ...
            $act['userid'] = Auth::user()->id;
            $act['ip'] = $request->ip();
            $act['activity'] = 'Update Profile Picture by Concern Department';
            $act['officecode'] = Auth::user()->dept_id;
            $act['pagereferred'] = $request->url();
            Activitylog::insert($act);
            if( !$upload ){
    ...
    if( !$validator->passes() ){
     ...
     $act['userid'] = Auth::user()->id;
     $act['ip'] = $request->ip();
     $act['activity'] = 'Update Change Password  by Concern Department';
     $act['officecode'] = Auth::user()->dept_id;
     $act['pagereferred'] = $request->url();
     Activitylog::insert($act);
     if( !$update ){
