...
class CsproDataController extends Controller
{
    public function index()
    {
        ...
        return view('dashboards.eva-dir.cspro-item', compact('cases', 'labelTitle'));
    }
    ...
    public function uploadCspro(Request $request)
    {
        try {
            ...
            if ($this->validateCsdbWithDcf($csdbPath)) {
                $cases = $this->processCsdbData($csdbPath);
            } else {
    ...
    public function processCsdbData($csdbFilePath)
    {
        
        // Validate CSDB file against DCF structure
        ...
        $labelNames = DB::table('imaster.item_labels')->pluck('item_label', 'item_name')->mapWithKeys(fn ($label, $key) => [strtolower($key) => $label]); // Convert keys to lowercase
        ...
        foreach ($cases as $case) {
                ...
                foreach ($tables as $table) {
                    ...
                    if ($data) {
                        if (isset($data->form_no)) {
                            $case->form_number = $data->form_no;
                        }
                        if (isset($data->person_id)) {
                            $case->person_id = $data->person_id;
                        }
                        ...
                        $recordNames = DB::table('imaster.item_labels')
                            ->pluck('record_name', 'item_name')
                            ->mapWithKeys(fn($record, $key) => [strtolower($key) => $record]); // Normalize keys
