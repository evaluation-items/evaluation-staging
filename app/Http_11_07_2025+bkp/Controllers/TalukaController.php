...
class TalukaController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $act['ip'] = $request->ip();
        $act['activity'] = 'Taluka Store By Admin';
    ...
    public function update(Request $request, Taluka $taluka)
    {
      ...
      if($taluka->tcode != "" && ($request->dcode != $taluka->dcode)){
        ...
        $act['userid'] = Auth::user()->id;
        $act['ip'] = $request->ip();
        $act['activity'] = 'Taluka Update By Admin';
        $act['officecode'] = Auth::user()->dept_id;
        $act['pagereferred'] = $request->url();
        Activitylog::insert($act);
     ...
     * @return \Illuminate\Http\Response
     */
    public function destroy(Request $request,Taluka $taluka)
    {
        if (!is_null($taluka)) {
            $taluka->delete();
            $act['userid'] = Auth::user()->id;
            $act['ip'] = $request->ip();
            $act['activity'] = 'Taluka Delete By Admin';
            $act['officecode'] = Auth::user()->dept_id;
            $act['pagereferred'] = $request->url();
            Activitylog::insert($act);
