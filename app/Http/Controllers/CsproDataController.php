...
class CsproDataController extends Controller
{
    ...
    public function caseFieldFiltter1(Request $request)
    {
        $t_id = $request->t_id;
        $selectedItems = $request->input('selected_items');
        $questionIds = $request->input('question_ids');
        ...
        if (!is_null($t_id) && $selectedItems) {
            ...
            $caseIds = DB::table('imaster.cspro_answer')
                ...
                return $this->getTheanswers($caseIds, $t_id, $html);
    ...
    function getTheanswers($caseIds,$t_id,$html){
     // Step 2: Load all related questions
            ...
            $html .= '<div class="table-responsive"><table id="example1" class="table table-bordered table-striped">';
            $html .= '<thead><tr>';
            ...
            if (!empty($groupedRows)) {
                foreach ($groupedRows as $caseId => $answersByQuestion) {
                    $html .= '<tr>';
                    foreach ($headers as $header) {
                        $answer = $answersByQuestion[$header][0] ?? 'No answer';

                        // Find questionId and answerId for this cell
                        // You need to get the corresponding questionId and answerId for this cell
                        $questionId = array_search($header, $questions->pluck('question', 'id')->toArray());
                        $answerObj = $allAnswers->where('cased_id', $caseId)
                                                ->where('question_id', $questionId)
                                                ->first();
                        $answer_id = $answerObj->id ?? null;

                        // Get note only for this question/answer/case
                        $remark_msg = '';
                        if ($questionId && $answer_id) {
                            $remark_msg = $this->getThenotesofQuestion($questionId, $caseId, $answer_id);
                        }

                        $html .= "<td>" . $answer;
                        if ($remark_msg) {
                            $html .= "<span hover-tooltip='" . $remark_msg . "' tooltip-position='top'><i class='fas fa-comment-dots' style='color: #3fa3dd;'></i></span>";
                        }
                        $html .= " </td>";
                    }
                    $html .= '</tr>';
                ...
                //         $html .= "<td>" . ($answer[0] ?? 'No answer') . "<span hover-tooltip='".$remark_msg."' tooltip-position='top'><i class='fas fa-comment-dots' style='color: #3fa3dd;'></i></span></td>";
                //     }
                //     $html .= '</tr>';
                // }
            } else {
    ...
    function getThenotesofQuestion($questionId, $caseId, $answer_id){
          $notes = DB::table('imaster.cspro_notes')
                            ->where('cased_id', $caseId)
                            ->where('cspro_answer_id', $answer_id)
                            ->where('question_id', $questionId)
                            ->select('notes')
                            ->latest()
                            ->first();
  ...
  public function caseFieldFiltter(Request $request)
 {
    try {
        ...
        // Step 1: Case IDs query
        $caseQuery = DB::table('imaster.cspro_answer as ca')
            ->where('ca.t_id', $t_id)
            ->when(!empty($inputQuestions), function ($q) use ($inputQuestions) {
                $q->whereIn('ca.question_id', array_keys($inputQuestions))
                  ->where(function ($sub) use ($inputQuestions) {
                      foreach ($inputQuestions as $questionId => $answers) {
                          $sub->orWhere(function ($sq) use ($questionId, $answers) {
                              $sq->where('ca.question_id', $questionId)
                                 ->whereIn('ca.answers', $answers);
                          });
                      }
                  });
            })
            ->select('ca.cased_id', 'ca.question_id');

        $allCaseIds = $caseQuery->get()
            ->groupBy('cased_id')
            ->filter(function ($group) use ($inputQuestions) {
                $matchedQuestions = $group->pluck('question_id')->unique()->toArray();
                return count(array_intersect(array_keys($inputQuestions), $matchedQuestions)) === count($inputQuestions);
        ...
        $totalRecords = $allCaseIds->count();

        // Paginate case IDs
        $caseIds = $allCaseIds->slice($start, $length)->values()->toArray();
        ...
        // Step 2: Load questions (headers)
        $questions = DB::table('imaster.questions')
            ->where('t_id', $t_id)
            ->get()
            ->keyBy('id');

        $headers = $questions->pluck('question')->toArray();

        // Step 3: Load answers for selected cases
        $allAnswers = DB::table('imaster.cspro_answer as ca')
            ->whereIn('ca.cased_id', $caseIds)
            ->where('ca.t_id', $t_id)
            ->get();

        // Step 4: Load options mapping
        ...
        $notes = DB::table('imaster.cspro_notes')
            ...
            ->select('cased_id', 'cspro_answer_id', 'question_id', 'notes')
            ->get()
            ->keyBy(fn ($note) => "{$note->cased_id}_{$note->cspro_answer_id}_{$note->question_id}");
        ...
        foreach ($allAnswers as $answer) {
            ...
            $remark_msg = $notes["{$caseId}_{$answer_id}_{$questionId}"]->notes ?? '';

            $cellContent = htmlspecialchars($optionText);
            if ($remark_msg) {
                $cellContent .= "<span hover-tooltip='" . htmlspecialchars($remark_msg) . "' tooltip-position='top'><i class='fas fa-comment-dots' style='color: #3fa3dd;'></i></span>";
            }

            $groupedRows[$caseId][$questionLabel] = $cellContent;
        }

        $data = [];
        foreach ($caseIds as $caseId) {
            $row = array_fill(0, count($headers), 'No answer');
            foreach ($headers as $i => $header) {
                if (isset($groupedRows[$caseId][$header])) {
                    $row[$i] = $groupedRows[$caseId][$header];
                }
            }
            $data[] = $row;
        }

        $columns = array_map(fn ($header) => ['title' => htmlspecialchars($header)], $headers);
        ...
        \Log::error('Error in caseFieldFilter: ' . $e->getMessage());
